...
import ru.niiar.cdr.model.QCallDetailRecord;
import ru.niiar.cdr.model.QSubscriber;
import ru.niiar.cdr.model.QUser;
...
public class QueryDslService{
    ...
    public List<Subscriber> getSubscriberByFilter(Subscriber subscriber){
        ...
        JPAQuery<?> query = queryFactory.from(sub);
                ...
                if (subscriber.getDivision().getId() != 0)
                    query.where(sub.division.id.eq(subscriber.getDivision().getId()));
    ...
    public List<Object[]> getPeopleDivisionCountCalls(Long id, String monthId, String year){
        if (monthId == null) {
            Query query = entityManager
    ...
    public List<Object[]> getRosDetalizationCountCalls(String monthId, String year) {
        if (monthId == null && year == null) {
            List<Object[]> result = entityManager
                    .createNativeQuery("select D.DIVISION_NAME, SUM(R.COST) as sum_cost from ros_detalizations R " +
    ...
    public List<Object[]> getPeopleRosDetalizationCountCalls(Long id, String monthId, String year) {
        if (monthId == null) {
            Query query = entityManager
    ...
    public List<Object[]> getAllRouteCount(String monthId, String year) {
        if (monthId == null && year == null) {
            List<Object[]> result = entityManager
                    .createNativeQuery("select ROUTE, SUM(R.COST) as sum_cost from ros_detalizations R " +
                            "inner join subscribers S on R.SUB_ID = S.SUB_ID " +
                            "inner join divisions D on S.DIVISION_ID = D.DIVISION_ID " +
                            "where to_char(DATE_TIME, 'MM-YYYY') = (select to_char(current_date, 'MM-YYYY') from dual) " +
                            "group by ROUTE " +
    ...
    public List<Object[]> getCountByRouteName(String route, String monthId, String year) {
        if (monthId == null) {
            Query query = entityManager
                    .createNativeQuery("select DIVISION_NAME, SUM(R.COST) as sum_cost from ros_detalizations R " +
                            "inner join subscribers S on R.SUB_ID = S.SUB_ID " +
                            "inner join divisions D on S.DIVISION_ID = D.DIVISION_ID " +
                            "where to_char(DATE_TIME, 'MM-YYYY') = (select to_char(current_date, 'MM-YYYY') from dual) " +
                            "AND ROUTE = ?1 " +
