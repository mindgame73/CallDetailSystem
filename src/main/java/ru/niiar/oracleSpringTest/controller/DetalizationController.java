    ...
    public String getChartView(@RequestParam(required = false) Integer id,
        ...
        if (id == null) {
            ...
            if (monthId == null && year == null)
                model.addAttribute("chartTitle",String.format("График затрат подразделений (Текущий месяц) | Общая сумма затрат составляет %s₽",all_sum));
            else
        ...
        else
        {
            ...
            if (monthId == null && year == null)
                model.addAttribute("chartTitle",String.format("Анализ затрат подразделения - %s (Текущий месяц) | Общая сумма затрат составляет %s₽",
                        selected.getDivisionName(),all_sum));
            else
